...
WifiMarauderScriptStageScan* _wifi_marauder_script_get_stage_scan(cJSON* stages) {
    ...
    cJSON* encryption = cJSON_GetObjectItem(stage_scan, "encryption");
    int scan_encryption = encryption != NULL ? (int)cJSON_GetNumberValue(encryption) : 0;
    ...
    scan_stage->encryption = scan_encryption; // Tilføjet
...
cJSON* _wifi_marauder_script_create_json_scan(WifiMarauderScriptStageScan* scan_stage) {
    ...
    // Encryption
    cJSON_AddNumberToObject(scan_json, "encryption", scan_stage->encryption); // Tilføjet
    return stage_json;
}
