...
void wifi_marauder_scan_stage_encryption_setup_callback(VariableItem* item) {
    WifiMarauderApp* app = variable_item_get_context(item);
    WifiMarauderScriptStageScan* stage = app->script_edit_selected_stage->stage;
    variable_item_set_current_value_index(item, stage->encryption);
}

void wifi_marauder_scan_stage_encryption_change_callback(VariableItem* item) {
    WifiMarauderApp* app = variable_item_get_context(item);
    ...
    stage_menu->items[3] = (WifiMarauderScriptMenuItem){
        ...
        .options =
            {"Open",
             ...
             "WAPI PSK",
             "Other"},
        .setup_callback = wifi_marauder_scan_stage_encryption_setup_callback,
